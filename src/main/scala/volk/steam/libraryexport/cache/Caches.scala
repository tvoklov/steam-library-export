...
object Caches {
  ...
  /**
   * caches the result of a "is this an actual game" check to a file. have to use this because on
   * big libraries steam complains that i'm making too many requests
   */
   ...
   */
  def buildCache[Key, Value](
      inputCache: MapFileCache[Key, Value]
    )(
      from: List[Key],
      using: Key => Client[IO] => IO[Value],
    )(
      implicit
      client: Client[IO],
    ...
    if notProcessed.isEmpty then
      IO.pure(inputCache)
    else {
      for {
        processed <- {
          scribe.info("querying steam...")
          IOOperations.runUntilOneCrashes(notProcessed.map(key => using(key)(client).map(key -> _)))
        }
        ...
        _ =
          scribe.info(
            if finished then
              ...
              s"not done querying, ${ notProcessed.size - processed.size }/${ from.size } queries left",
          )
        ...
        res <-
          if finished then
            ...
            IO.sleep(
              if processed.size < 15 then
                1.minutes // most probably steam's timeout machine just wasn't fast enough for me
              else
