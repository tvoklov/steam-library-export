...
object GamesWithScores {
  ...
  private val headerRows =
    List(
      Row(style = CellStyle(fillBackgroundColor = Color.Yellow))
        .withCellValues(List("Name", "Minutes Played", "Review Score", "Total reviews"))
    )
  ...
  def make(
      gamesWithScores: List[(GameInfo, ReviewStats)]
    ): Sheet = {
    def makeRows: List[Row] =
      gamesWithScores
        ...
        .map { case (gi, rs) =>
          Row().withCellValues(
            List(gi.name, gi.playtime.allPlaytime, rs.positiveReviewPercentage, rs.totalReviewCount)
          )
        }

    Sheet(name = "Game review scores").withRows(headerRows).withColumns(columns).addRows(makeRows)
  }
